...
% Calculate array of points of intersection of lines corresponding to angular voxels and 
% largest radius
P = [];
k = 0;
while k < 2 *pi
...
% Find the point of intersection of the ray and the largest radius
l = sqrt(ray_direction(1)^2 + ray_direction(2)^2);
p = circle_center  - (circle_max_radius/l) * (circle_center - ray_origin);
for i = 1:length(P)
    if p(2) < abs(P(i,2)) && sign(p(2)) == sign(P(i,2)) && sign(p(1)) == sign(P(i,1))
    theta = i;     
    end
end
%theta
