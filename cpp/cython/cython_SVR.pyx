...
def walk_spherical_volume(np.ndarray[np.float64_t, ndim=1, mode="c"] ray_origin,
                          ...
                          np.ndarray[int, ndim=1, mode="c"] voxel_min_bound,
                          np.ndarray[int, ndim=1, mode="c"] voxel_max_bound,
    ...
    Arguments:
           ...
           // TODO(cpg49): Update these to include examples.
           voxel_min_bound: The minimum boundary of intersection.
           voxel_max_bound: The maximum boundary of intersection.
    ...
    assert(voxel_min_bound.size == 3)
    assert(voxel_max_bound.size == 3)

    cdef vector[SphericalVoxel] voxels = walkSphericalVolume(&ray_origin[0], &ray_direction[0],
                                                             &voxel_min_bound[0], &voxel_max_bound[0],
