...
cpp_title = "RayBeginsAtSphereOrigin"
python_title = "test_ray_begins_at_sphere_origin"
...
ray_dir = np.array([-1.5, 1.2, -1.5])
t_begin = 0.0
t_end = 30.0
expected_radial_voxels = str([4,3,2,1])[1:-1]
expected_angular_voxels = str([1,1,1,1])[1:-1]
expected_azimuthal_voxels = str([2,2,2,2])[1:-1]
...
print("    const svr::SphericalVoxelGrid grid(min_bound, max_bound, num_radial_sections, num_angular_sections,")
print("                                       num_azimuthal_sections, sphere_center, sphere_max_radius);")
